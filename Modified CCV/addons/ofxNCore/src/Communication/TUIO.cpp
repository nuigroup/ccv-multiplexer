...
void TUIO::sendTUIO(std::map<int, Blob> * fingerBlobs, std::map<int, Blob> * objectBlobs ,std::list <ofxFiducial> * fiducialsList)
{
	...
	if(bOSCMode)
	{
		if(bFingers)
		{
			...
			alive.setAddress(version ? "/tuio/2Dblb" : "/tuio/2Dcur");
			alive.addStringArg("alive");
	
			// Send fseq message
			ofxOscMessage fseq;
			fseq.setAddress( version ? "/tuio/2Dblb" : "/tuio/2Dcur" );
			...
			else // actually send the blobs
			{
				if(bFingers)
				{
					...
					for(blob = fingerBlobs->begin(); blob != fingerBlobs->end(); blob++)
					{
						...
						//Set Message
						ofxOscMessage set;
						set.setAddress( version ? "/tuio/2Dblb" : "/tuio/2Dcur"  );
						...
						if (version)
						{
							set.addFloatArg(blob->second.angle);				//a
							set.addFloatArg(blob->second.angleBoundingRect.width/width);	//w
							set.addFloatArg(blob->second.angleBoundingRect.height/height);	//h
							set.addFloatArg(blob->second.area);					//f
							set.addFloatArg(blob->second.D.x);					//X
							set.addFloatArg(blob->second.D.y);					//Y
